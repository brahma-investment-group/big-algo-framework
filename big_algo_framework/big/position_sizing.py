class PositionSizing():
    def __init__(self, order_dict):
        ...
        self.option_multiplier = order_dict["multiplier"]
    ...
    def options_quantity(self):
        if self.total_risk_units == "amount":
            self.quantity = int(self.total_risk / (self.risk_unit * self.option_multiplier))

        if self.total_risk_units == "percent":
            self.quantity = int(self.total_risk * self.available_capital * 0.01 / (self.risk_unit * self.option_multiplier))
            cost = self.quantity * self.entry * self.option_multiplier

            if cost > self.max_cost:
                self.quantity = int(self.max_cost / (self.entry * self.option_multiplier))
