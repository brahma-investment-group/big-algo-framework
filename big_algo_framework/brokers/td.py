...
class TDA(Broker):
    ...
    def get_all_positions(self, order_dict):
        ...
        return response.json()
        #TODO: change order_dict to position_dict
    ...
    def cancel_order(self, order_id, order_dict):
        ...
        #TODO: change order_dict to position_dict. change in IB aswell
...
# get quotes (STK or OPT Symbols)... Client.get_quotes(symbols)
    ...
    def get_trailing_stop_order(self, order_dict, digits = 2):
        ...
        if sec_type == "STK":
            if instruction == "OPEN":
                if action == "BUY":
                    trailing_stop_order = equity_buy_market(symbol, quantity).set_order_type(OrderType.TRAILING_STOP).set_stop_price_link_basis(orders.common.StopPriceLinkBasis.MARK).set_stop_price_link_type(price_link).set_stop_price_offset(offset).set_duration(tda_tif)
                elif action == "SELL":
                    trailing_stop_order = equity_sell_short_market(symbol, quantity).set_order_type(OrderType.TRAILING_STOP).set_stop_price_link_basis(orders.common.StopPriceLinkBasis.MARK).set_stop_price_link_type(price_link).set_stop_price_offset(offset).set_duration(tda_tif)
            elif instruction == "CLOSE":
                if action == "BUY":
                    trailing_stop_order = equity_buy_to_cover_market(symbol, quantity).set_order_type(OrderType.TRAILING_STOP).set_stop_price_link_basis(orders.common.StopPriceLinkBasis.MARK).set_stop_price_link_type(price_link).set_stop_price_offset(offset).set_duration(tda_tif)
                elif action == "SELL":
                    trailing_stop_order =equity_sell_market(symbol, quantity).set_order_type(OrderType.TRAILING_STOP).set_stop_price_link_basis(orders.common.StopPriceLinkBasis.MARK).set_stop_price_link_type(price_link).set_stop_price_offset(offset).set_duration(tda_tif)

        elif sec_type == "OPT":
            if instruction == "OPEN":
                if action == "BUY":
                    trailing_stop_order = option_buy_to_open_market(symbol, quantity).set_order_type(OrderType.TRAILING_STOP).set_stop_price_link_basis(orders.common.StopPriceLinkBasis.MARK).set_stop_price_link_type(price_link).set_stop_price_offset(offset).set_duration(tda_tif)
                elif action == "SELL":
                    trailing_stop_order = option_sell_to_open_market(symbol, quantity).set_order_type(OrderType.TRAILING_STOP).set_stop_price_link_basis(orders.common.StopPriceLinkBasis.MARK).set_stop_price_link_type(price_link).set_stop_price_offset(offset).set_duration(tda_tif)
            elif instruction == "CLOSE":
                if action == "BUY":
                    trailing_stop_order = option_buy_to_close_market(symbol, quantity).set_order_type(OrderType.TRAILING_STOP).set_stop_price_link_basis(orders.common.StopPriceLinkBasis.MARK).set_stop_price_link_type(price_link).set_stop_price_offset(offset).set_duration(tda_tif)
                elif action == "SELL":
                    trailing_stop_order = option_sell_to_close_market(symbol, quantity).set_order_type(OrderType.TRAILING_STOP).set_stop_price_link_basis(orders.common.StopPriceLinkBasis.MARK).set_stop_price_link_type(price_link).set_stop_price_offset(offset).set_duration(tda_tif)
