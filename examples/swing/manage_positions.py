...
class managePositions(Strategy):
    ...
    async def before_send_orders(self):
        current_positions = await self.broker.get_all_positions(account_no=self.account_no)
        print(current_positions)
        # print(self.broker.reqPnL(self.account_no))
        # print(self.broker.pnl(self.account_no))
        # print(self.broker.pnlSingle(self.account_no, '', 555101484 ))

        print(self.broker.reqPnL(account=self.account_no))
        time.sleep(5)
        for PnL in self.broker.pnl(account=self.account_no):
