...
class managePositions(Strategy):
    ...
    async def send_orders(self):
        while True:
            time.sleep(10)
            print("************")
            for pos in self.broker.portfolio():
                self.quantity = pos.position
                self.ticker = pos.contract.symbol
                self.exchange = "SMART"
                self.primary_exchange = pos.contract.primaryExchange
                self.currency = pos.contract.currency
                self.expiration_date = pos.contract.lastTradeDateOrContractMonth
                self.strike = pos.contract.strike
                self.right = pos.contract.right
                self.multiplier = pos.contract.multiplier
                self.avg_price = pos.averageCost
