...
async def mkt_order():
    # MKT ORDER
    order = await broker.get_market_order(symbol=ticker, quantity=1, sec_type="STK", action='BUY', session='NORMAL', duration='DAY')
    await broker.send_order(account_no=account_no, order=order)

async def stp_lmt_order():
    # STOP LIMIT ORDER
    order = await broker.get_stop_limit_order(symbol=ticker, quantity=1, sec_type="STK", stop_price=6.25, limit_price=6.25, action='BUY', session='NORMAL', duration='DAY', stop_type='MARK')
    await broker.send_order(account_no=account_no, order=order)

async def lmt_order():
    # LIMIT ORDER
    order = await broker.get_limit_order(symbol=ticker, quantity=1, sec_type="STK", limit_price=3.25, action='BUY', session='NORMAL', duration='DAY')
    await broker.send_order(account_no=account_no, order=order)

async def stp_order():
    # STOP ORDER
    order = await broker.get_stop_order(symbol=ticker, quantity=1, sec_type="STK", stop_price=6.25, action='BUY', session='NORMAL', duration='DAY', stop_type='MARK')
    await broker.send_order(account_no=account_no, order=order)

async def trailing_stp_percentage_order():
    # TRAILING STOP ORDER (PERCENTAGE)
    order = await broker.get_trailing_stop_order(symbol=ticker, quantity=1, sec_type="STK", trailing_offset=5, action='BUY',
...
async def trailing_stp_lmt_percentage_order():
    ...
    order = await broker.get_trailing_stop_limit_order(symbol=ticker, quantity=1, sec_type="STK", trailing_offset=5, limit_price=6, action='BUY',
                                                       stop_price_link_type='PERCENT', stop_price_link_basis='MARK', stop_type='MARK',
                                                       session='NORMAL', duration='DAY')
    await broker.send_order(account_no=account_no, order=order)
...
async def oco_order():
    # OCO ORDER
    order1 = await broker.get_market_order(symbol=ticker, quantity=1, sec_type="STK", action='BUY')
    order2 = await broker.get_limit_order(symbol=ticker, quantity=1, sec_type="STK", limit_price=6.25, action='BUY')
