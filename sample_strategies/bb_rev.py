...
import pytz
from strategies.strategy_rules import strategyRules
...
class BbRev(Strategy):
    def __init__(self, broker, ticker, db, historic_data_table, streaming_data_table, order_dict):
        ...
        self.no_bb = 21
        self.min_candles_ltf = min(self.no_bb)
    ...
    def ftfc(self, direction):
        ftfcList = []

        for i in range(len(self.entry_conditions)):
            for j in range(len(self.entry_conditions[i]["FTFC"])):
                tfc_tf = self.entry_conditions[i]["FTFC"][j]
                ftfc_resample = resample(self.db,
                                         [self.ticker],
                                         self.tf_props[tfc_tf]["base_timeframe"],
                                         tfc_tf,
                                         self.tf_props[tfc_tf]["rule"],
                                         self.historic_data_table,
                                         self.streaming_data_table,
                                         self.tf_props[self.tf_props[tfc_tf]["base_timeframe"]]["rule"],
                                         self.tf_props[tfc_tf]["is_origin"])
                ftfc_df = ftfc_resample.resample_price()

                if len(ftfc_df) > 1:
                    ftfc = strategyRules([self.ticker], ftfc_df)

                    if direction == "Bullish":
                        ftfcList.extend([ftfc.becomes_ftfc_at_price(ftfc_df['high'].iloc[-2])])
                    elif direction == "Bearish":
                        ftfcList.extend([ftfc.becomes_ftfc_at_price(ftfc_df['low'].iloc[-2])])

        if all(tfc == direction for tfc in ftfcList):
            return True
        else:
            return False
    ...
    def check_conditions(self, direction):
        ...
        for i in range(len(self.entry_conditions)):
            ...
            if len(ltf_df) >= self.min_candles_ltf:
                ...
                bullish_creteria = [ltf_df["low"].iloc[-2] < bb_df["LBB"].iloc[-2], #price lower than LBB
                                    self.ftfc(direction)]                           #FTFC to the upside

                bearish_creteria = [ltf_df["high"].iloc[-2] > bb_df["UBB"].iloc[-2], #price higher than UBB
                                    self.ftfc(direction)]                           #FTFC to the downside
        ...
        for dic in order_results_dict:
            ...
            if direction == "Bullish":
                ...
                if current_sl < new_SL:
                    #Modify the order
                    order_dict = {"so_order_id": order_id,
                                  "so_action": action,
                                  "so_quantity": quantity,
                                  "s0_stop_price": new_SL,
                                  "so_time_in_force": "GTC",
                                  "so_good_till_date": "",
                                  "so_transmit": True
                                  }
                    ...
                    self.broker.send_order(self, order_dict, stoploss_order)
    ...
    def start(self):
        self.broker.init_client(self.broker, self.order_dict)
        self.con = self.broker.get_contract()
